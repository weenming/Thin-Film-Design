...
def insert_1_layer(wls, target_spec, d, materials, insert_search_pts):
    ...
    """
    insert_gradient = np.dot(get_insert_jacobi.get_insert_jacobi_faster(wls, d, materials, insert_search_pts).T,
...
def main():
    ...
    wls = np.linspace(220, 1700, 1481)
    target_spectrum_3column = np.loadtxt(
        open("oic2022-PA-Measurements.csv", "rb"),
        delimiter=",", skiprows=1)
    # 合并R和T光谱
    target_spec = np.vstack(
        (np.array([target_spectrum_3column[0:, 2]]).T, np.array([target_spectrum_3column[0:, 1]]).T)) / 100  # 百分制变成小数
    ...
    # d = np.loadtxt('count1_layer3_merit0.21877475000923963.txt')
    d = np.array([2000, 1000, 2000], dtype='double')
    materials = np.array([])
    ...
    for insert_count in range(100):
        ...
        np.savetxt(f'generated_data\\OIC A1 from 1000 1000 1000\\start5000nm\\count{insert_count+1}_layer{layer_number}_merit{merit}.txt', d)
        np.savetxt(f'generated_data\\OIC A1 from 1000 1000 1000\\start5000nm\\count{insert_count+1}_materials.txt', materials, fmt='%s')
