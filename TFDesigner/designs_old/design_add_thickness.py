...
fpath = 'training_plot_add_thickness'
...
def descent_process(d, materials, merit, target_optical_thickness, merits=np.array([]), layer_numbers=np.array([]),
                    optical_thickness_ratios=np.array([]), do_plot=False):
    """
    ...
    :param merits:
    :param layer_numbers:
    :param optical_thickness_ratios:
    :param do_plot:
    :return:
    """
    merits = np.append(merits, merit)
    layer_numbers = np.append(layer_numbers, d.shape[0])
    optical_thickness_ratios = np.append(optical_thickness_ratios, calculate_optical_thickness(d, materials) / target_optical_thickness)
    ...
    plt.savefig(f'../results/{fpath}/training_plot_{start_time}.png')
    plt.close('all')
    np.savetxt(f'../results/{fpath}/merits_{start_time}.txt', merits)
    np.savetxt(f'../results/{fpath}/layers_{start_time}.txt', layer_numbers)

    if do_plot:
        plt.show()

    def next_descent_process(d_next, materials_next, merit_next, do_plot_next=False):
        return descent_process(d_next, materials_next, merit_next, target_optical_thickness, merits, layer_numbers,
                               optical_thickness_ratios, do_plot_next)
...
class InitializeParameters:
    ...
    def init_init_layers(self):
        ...
        for i in range(d.shape[0]):
            materials = np.append(materials, available_materials[(i+1) % 2])
    ...
    def init_init_layers_from_file(self, d_file_path, materials_file_path):
        ...
        assert d.shape == materials.shape, 'd and materials do not align in shape'
...
def main():
    ...
    d, materials = current_initialize.init_init_layers_from_file('count45_layer37_merit0.0018148406899898316.txt', 'count45_materials.txt')
    ...
    merits, descent_process_this = descent_process(d, materials, merit, current_initialize.target_optical_thickness)
    ...
    for insert_count in range(100):
        ...
        if merits.shape[0] > 3 and np.abs(merits[-1] - merits[-3])/ merits[-1] < 1e-2:
            d, materials = insert_in_the_end(d, materials, 100., 'SiO2')
            print('additional thickness added')

        np.savetxt(f'..\\results\\{fpath}\\generated_data\\count{insert_count+1}_layer{d.shape[0]}_merit{merit}.txt', d)
        np.savetxt(f'..\\results\\{fpath}\\generated_data\\count{insert_count+1}_materials.txt', materials, fmt='%s')
