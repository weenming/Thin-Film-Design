from gets.get_jacobi import get_jacobi_multi_inc
from gets.get_spectrum import get_spectrum_multi_inc
from gets.get_n import get_n
from gets.get_insert_jacobi import get_insert_jacobi_faster_multi_inc
...
folder_name = 'merit_thickness_layer_2'
fpath = f'./../results/{folder_name}/'
if not os.path.exists(fpath):
    os.mkdir(fpath)
if not os.path.exists(fpath+'generated_data'):
    os.mkdir(fpath + 'generated_data')

inc_angles = np.array([60., 0.])
...
def plot_spectrum(wls, target_spec, d, materials, layer_number, merit=None, insert_count=None):
    if merit is None:
        calculate_merit(d, materials, wls, target_spec)
    """
    ...
    fig_spec, (ax_1, ax_2) = plt.subplots(2, 2, sharey=True)
    axR, axT = ax_1
    axR_2, axT_2 = ax_2

    axR.plot(wls, get_spectrum_multi_inc(wls, d, materials, theta0=inc_angles)
             [0:wls.shape[0]], label='fit')
    axR.plot(wls, target_spec[0:wls.shape[0]], label='target')
    axR.legend()
    axR.set_xlabel('wl/nm')
    axR.set_title(f'R')

    axT.plot(wls, get_spectrum_multi_inc(wls, d, materials, theta0=inc_angles)
             [wls.shape[0]: wls.shape[0] * 2], label='fit')
    axT.plot(wls, target_spec[wls.shape[0]: wls.shape[0] * 2], label='target')
    axT.legend()
    axT.set_xlabel('wl/nm')
    axT.set_title('T')


    axR_2.plot(wls, get_spectrum_multi_inc(wls, d, materials, theta0=inc_angles)
             [wls.shape[0] * 2: wls.shape[0] * 3], label='fit')
    axR_2.plot(wls, target_spec[wls.shape[0] * 2: wls.shape[0] * 3], label='target')
    axR_2.legend()
    axR_2.set_xlabel('wl/nm')
    axR_2.set_title(f'R')

    axT_2.plot(wls, get_spectrum_multi_inc(wls, d, materials, theta0=inc_angles)
             [wls.shape[0] * 3: wls.shape[0] * 4], label='fit')
    axT_2.plot(wls, target_spec[wls.shape[0] * 3: wls.shape[0] * 4], label='target')
    axT_2.legend()
    axT_2.set_xlabel('wl/nm')
    axT_2.set_title('T')

    fig_spec.suptitle(f'{layer_number} layers, merit func = {merit}')
    fig_spec.savefig('test')
...
def descent_process(d, materials, merit, target_optical_thickness, ratio, merits=np.array([]), layer_numbers=np.array([]),
    ...
    optical_thicknesses = np.append(
        optical_thicknesses, calculate_optical_thickness(d, materials))

    fig, ax1 = plt.subplots()
    ax2 = ax1.twinx()
    ax1.plot(range(merits.shape[0]), np.log(
        merits), label='merits', color='orange')
    ax2.plot(range(merits.shape[0]), layer_numbers,
             label='layer number', color='steelblue')
    ax1.legend(loc='upper right')
    ax2.legend(loc='upper left')
    fig.suptitle('descent_process')
    plt.savefig(
        fpath + f'ratio_{ratio}_training_plot_{start_time}.png')
    plt.close('all')
    np.savetxt(
        fpath + f'ratio_{ratio}_merits_{start_time}.txt', merits)
    np.savetxt(
        fpath + f'ratio_{ratio}_layers_{start_time}.txt', layer_numbers)
    np.savetxt(
        fpath + f'ratio_{ratio}_ots_{start_time}.txt', optical_thicknesses)
...
def gradient_descent(wls, target_spec, d, materials, insert_layer_num):
    ...
    J = get_jacobi_multi_inc(wls, d, materials, theta0=inc_angles)
    f = get_spectrum_multi_inc(wls, d, materials, theta0=inc_angles) - target_spec
    g = np.dot(J.T, f)
    A = np.dot(J.T, J)
    nu = 2
    mu = 1
    ...
    for gradient_descent_count in range(10000):
        ...
        spectrum_new = get_spectrum_multi_inc(wls, d_new, materials, theta0=inc_angles)
        ...
        if np.amax(abs(h)).item() < 1e-10 and negative_count == 0:
            merit = np.sqrt(((get_spectrum_multi_inc(
                wls, d, materials, theta0=inc_angles) - target_spec) ** 2 / wls.shape[0]).sum())
...
def insert_1_layer(wls, target_spec, d, materials, insert_search_pts):
    ...
    insert_gradient = np.dot(
        get_insert_jacobi_faster_multi_inc(
            wls, d, materials, insert_search_pts, theta0=inc_angles).T,
        get_spectrum_multi_inc(wls, d, materials, theta0=inc_angles) - target_spec)
...
class InitializeParameters:
    ...
    def init_target_layers(self, do_plot_target_spec=False):
        ...
        for i in range(d_target.shape[0]):
            materials_target = np.append(
                materials_target, self.available_materials[i % 2])
        # Target optical thickness
        self.target_optical_thickness = calculate_optical_thickness(
            d_target, materials_target, self.test_wl)
        # Generate and plot target spectrum (R)
        wls = np.linspace(500, 1000, 500)
        target_spec = get_spectrum_multi_inc(wls, d_target, materials_target, theta0=inc_angles)
        if do_plot_target_spec:
            plt.plot(wls, target_spec[0:wls.shape[0]], label=f'target spectrum, {inc_angles[0]} deg')
            plt.plot(wls, target_spec[wls.shape[0] * 2: wls.shape[0] * 3], label=f'target spectrum, {inc_angles[1]} deg')
            plt.legend()
            plt.xlabel('wl/nm')
            plt.show()
    ...
    def init_init_layers(self):
        ...
        test_optical_thickness = calculate_optical_thickness(
            d, materials, self.test_wl)
        initial_thickness = self.initial_optical_thickness * \
            (test_thickness / test_optical_thickness)
        d[0] = initial_thickness

        return d, materials

    def init_init_layers_from_file(self, d_file_path, materials_file_path):
        assert os.path.exists(d_file_path) and os.path.exists(
            materials_file_path), 'file does not exist'
        d = np.loadtxt(d_file_path)
        materials = np.loadtxt(materials_file_path, dtype='str')
        self.initial_optical_thickness = calculate_optical_thickness(
            d, materials)
        return d, materials
...
def main(ratio):
    ...
    print(calculate_optical_thickness(d, materials))

    # Initialize optimization parameters
    insert_layer_num = 0
    merit = calculate_merit(d, materials, wls, target_spec)
    # Initialize training process record function
    merits, descent_process_this = descent_process(
        d, materials, merit, current_initialize.target_optical_thickness, ratio)

    for insert_count in range(100):
        # 梯度下降
        d, materials, insert_layer_num, merit = gradient_descent(
            wls, target_spec, d, materials, insert_layer_num)

        # record the training process: plot and save figure, save training merits
        merits, descent_process_this = descent_process_this(
            d, materials, merit)
        plot_spectrum(wls, target_spec, d, materials, d.shape[0])
        # 插入新层
        # last_insert_layer_thickness = d[insert_layer_num]
        insert_search_pts = 100
        d, materials, insert_layer_num = insert_1_layer(
            wls, target_spec, d, materials, insert_search_pts)
        ...
        np.savetxt(
            fpath + f'generated_data/ratio{ratio}_count{insert_count+1}_layer{d.shape[0]}_merit{merit}.txt', d)
        np.savetxt(
            fpath + f'generated_data/ratio{ratio}_count{insert_count+1}_materials.txt', materials, fmt='%s')
