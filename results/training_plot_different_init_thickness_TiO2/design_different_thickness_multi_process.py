...
folder_training_process = 'training_plot_different_init_thickness_1_TiO2'
folder_training_data = 'different_init_thickness_1_TiO2'
...
def descent_process(layer_number, merit, merits=np.array([]), layer_numbers=np.array([]), optical_thickness_ratio=None,
    ...
    ax1.set_title(f'ratio{optical_thickness_ratio}')
    plt.savefig(f'../results/{folder_training_process}/merit_to_layer_thickness_ratio_{optical_thickness_ratio}.png')
    if do_plot:
        plt.show()
...
class initializeParameters:
    ...
    def init_init_layers(self):
        ...
        assert self.target_optical_thickness is not None, 'target not generated'
...
def main(ratio):
    ...
    for insert_count in range(50):
        ...
        np.savetxt(f'..\\results\\generated_data\\{folder_training_data}\\ratio_{ratio}_count{insert_count+1}_layer{layer_number}_merit{merit}.txt', d)
        np.savetxt(f'..\\results\\generated_data\\{folder_training_data}\\ratio_{ratio}_count{insert_count+1}_materials.txt', materials, fmt='%s')

        print(f'ratio={ratio}, merits={merit_after_50_iters}')
        merit_after_50_iters = np.append(merit_after_50_iters, merit)
    np.savetxt(f'../results/{folder_training_process}/merit_after_each_insertions_ratio{ratio}.txt', merit_after_50_iters)

    end = time.time()
    print(f'design starting ratio {ratio} takes {end - start}')
    return


if __name__ == '__main__':
    ratio_num = 15
    ratios = np.linspace(0.5, 2.0, ratio_num)
    np.savetxt(f'../results/{folder_training_process}/optic_thickness_ratios.txt', ratios)
