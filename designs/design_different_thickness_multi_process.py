...
def descent_process(layer_number, merit, merits=np.array([]), layer_numbers=np.array([]), optical_thickness_ratio=None,
    ...
    fig.title(f'ratio{optical_thickness_ratio}')
...
def main(ratio):
    ...
    for insert_count in range(50):
        ...
        np.savetxt(f'..\\results\\generated_data\\different_init_thickness\\ratio_{ratio}_count{insert_count+1}_layer{layer_number}_merit{merit}.txt', d)
        np.savetxt(f'..\\results\\generated_data\\different_init_thickness\\ratio_{ratio}_count{insert_count+1}_materials.txt', materials, fmt='%s')

        print(merit_after_50_iters)
        merit_after_50_iters = np.append(merit_after_50_iters, merit)
    np.savetxt(f'../results/training_plot_different_init_thickness/merit_after_50_insertions_ratio{ratio}.txt', merit_after_50_iters)
