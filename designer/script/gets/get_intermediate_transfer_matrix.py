...
def _launch_propagation(W_spec, wls, d, n_layers, n_sub, n_inc, inc_ang, 
    ...
    Ws_device = cuda.device_array((wls_size * 2, 2, 2), dtype="float64")
...
def get_E_at_i(E_arr, wls, d, n_layers, n_sub, n_inc, inc_ang):
    return
...
def forward_propagation_simple_W_i(W_spec, wls, d, n_A_arr, n_B_arr,
    ...
    i += 1 # if inc, i should start with 0 later; if not, 
    ...
    W_spec[thread_id, :, :] = Ws # s-polarized


    W_spec[thread_id + wls_size, :, :] = Wp # p-polarized
