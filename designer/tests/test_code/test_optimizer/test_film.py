...
class TestFilm(unittest.TestCase):
    ...
    def test_insert(self):
        f = FilmSimple('SiO2', 'TiO2', 'SiO2', np.array([1, 2, 3], dtype='float'))

        f_tmp = copy.deepcopy(f)
        self.assertRaises(AssertionError, lambda: f_tmp.insert_layer(0, -0.5, 100))
        f_tmp = copy.deepcopy(f)
        self.assertRaises(AssertionError, lambda: f_tmp.insert_layer(3, 0.5, 100))
        ...
        # single layer
        f = FilmSimple('SiO2', 'TiO2', 'SiO2', np.array([1], dtype='float'))
