...
class TestJacobi(unittest.TestCase):
    ...
    def test_film_jacobi_many_layers(self):
        ...
        stack_J(jacobi, [[f.spectrum[0].n, f.spectrum[0].n_sub, f.spectrum[0].n_inc]], f.get_d(), f.get_all_spec_list())

        # read expected spec from file
        # count relative path from VS Code project root.....                                                        
        expected_jacobi = np.loadtxt("./designer/tests/test_files/expected_jacobi_simple_R_500to1000_1000layer_SiO2-TiO2-times-500-SiO2_60inc.csv", dtype="float")
...
def make_var_jacobi():
    ...
    for j in range(jacobi.shape[1]):
        ...
        get_spectrum_simple(R_1, wls, d, f.get_spec().n, f.get_spec().n_sub, f.get_spec().n_inc, inc_ang)
        d[j] += h
        get_spectrum_simple(R_2, wls, d, f.get_spec().n, f.get_spec().n_sub, f.get_spec().n_inc, inc_ang)
