...
class TestFilm(unittest.TestCase):
    ...
    def test_film_change_d(self):
        ...
        self.assertAlmostEqual(
            np.max(np.abs(f.calculate_n_array(wls) - n_expected)), 0)
    ...
    def test_film_spectrum(self):
        ...
        expected_spec = np.loadtxt(
            "./designer/tests/test_files/expected_spectrum_simple_R_500to1000_30layer_SiO2-TiO2-times-15-SiO2_60inc.csv", dtype="float")
        self.assertAlmostEqual(
            np.max(np.abs(f.spectrums[0].get_R() - expected_spec)), 0)
        self.assertAlmostEqual(
            np.max(np.abs(1 - f.spectrums[0].get_T() - expected_spec)), 0)

    def test_one_layer(self):
        substrate = A = "SiO2"
        B = "TiO2"
        d = np.array([1000])
        f = film.TwoMaterialFilm(B, A, substrate, d)
        # must set spec before calculating spec
        inc_ang = 60.  # incident angle in degree
        wls = np.linspace(500, 1000, 500)
        f.add_spec_param(inc_ang, wls)
        ...
        # read expected spec from file
        # count relative path from VS Code project root.....
        expected_spec = get_spectrum_cpu.get_spectrum(
            wls, d, np.array([B]), theta0=inc_ang)

        self.assertAlmostEqual(
            np.max(np.abs(f.get_spec().get_R() - expected_spec[:wls.shape[0], 0])), 0)
        self.assertAlmostEqual(
            np.max(np.abs(1 - f.get_spec().get_T() - f.get_spec().get_R())), 0)
