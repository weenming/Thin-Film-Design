...
fpath = 'training_plot_add_thickness_SiO2'
...
def main():
    ...
    d, materials = current_initialize.init_init_layers_from_file('../results/'+fpath+'/generated_data/count100_layer72_merit1.5120583884296617e-05.txt', '../results/'+fpath+'/generated_data/count100_materials.txt')
    ...
    for insert_count in range(100):
        ...
        np.savetxt(f'..\\results\\{fpath}\\generated_data\\count{insert_count+101}_layer{d.shape[0]}_merit{merit}.txt', d)
        np.savetxt(f'..\\results\\{fpath}\\generated_data\\count{insert_count+101}_materials.txt', materials, fmt='%s')
