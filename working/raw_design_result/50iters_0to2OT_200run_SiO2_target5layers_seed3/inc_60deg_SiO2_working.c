...
DWORD WINAPI run(LPVOID args_ptr) {
    ...
    sample_pts_num = File_Size("generaetd_spectrum-INC_ANG60.0-WLS500.0to1000.0-R_5layers_seed3.txt");
    target_spec = VectorD_New(sample_pts_num);
    VectorD_ReadFromFile(target_spec, "generaetd_spectrum-INC_ANG60.0-WLS500.0to1000.0-R_5layers_seed3.txt");
    ...
    char fpath[200];
    sprintf(fpath, "./../result/5layers_SiO2_0to2_seed3/run_%d/init", run_count);
    ...
    for (i = 0; i < 50; i++) {
        ...
        d = VectorD_New(Film_GetOptSize(film));
        Film_GetOptParam(film, d);
        sprintf(fpath, "./../result/5layers_SiO2_0to2_seed3/run_%d/iter_%d", run_count, i);
        VectorD_WriteToFile(d, fpath);
        printf("thread %d: %d-th iteration finished\n", run_count, i);
    }
    sprintf(fpath, "./../result/5layers_SiO2_0to2_seed3/run_%d/final", run_count);
    VectorD_WriteToFile(d, fpath);
...
int main() {
    ...
    VectorD_MulNumD(ratios, 2.30, ratios); // SiO2
