...
DWORD WINAPI run(LPVOID args_ptr) {
    ...
    sample_pts_num = File_Size("generaetd_spectrum-INC_ANG60.0-WLS500.0to1000.0-R_8layers.txt");
    target_spec = VectorD_New(sample_pts_num);
    VectorD_ReadFromFile(target_spec, "generaetd_spectrum-INC_ANG60.0-WLS500.0to1000.0-R_8layers.txt");
    ...
    char fpath[200];
    sprintf(fpath, "./../result/8layers_TiO2_0to2/run_%d/init", run_count);
    ...
    for (i = 0; i < 50; i++) {
        ...
        d = VectorD_New(Film_GetOptSize(film));
        Film_GetOptParam(film, d);
        sprintf(fpath, "./../result/8layers_TiO2_0to2/run_%d/iter_%d", run_count, i);
        VectorD_WriteToFile(d, fpath);
        printf("thread %d: %d-th iteration finished\n", run_count, i);
    }
    sprintf(fpath, "./../result/8layers_TiO2_0to2/run_%d/final", run_count);
    VectorD_WriteToFile(d, fpath);
...
int main() {
    ...
    // VectorD_MulNumD(ratios, 1.21, ratios); // SiO2
    VectorD_MulNumD(ratios, 0.728, ratios); // TiO2 don't forget to change the starting material
