...
def plot_descent_process(layer_number, merit, merits=np.array([]), layer_numbers=np.array([])):
    ...
    fig.suptitle('10layer_0degree_start5000nm')
    fig.show()
...
def insert_1_layer(wls, target_spec, d, materials, insert_search_pts):
    ...
    # plt.xlabel('d/nm')
    # plt.legend()
    # plt.title(f'insert gradient, layer num={layer_number}, 10layer_0degree')
    # plt.show()
    # 找最小梯度的地方插入一层
...
def main():
    # 先生成目标的光谱：总共5层
    x = 1000
    d_init = np.array([123, 456, 789, 234, 567, 891, 234, 567, 789, 345], dtype='double')
    ...
    # d = np.loadtxt('10layer_0degree_before_adjusting_merit0.09939537983453904.txt')
    d = np.array([5000], dtype='double')
    materials = np.array([])
    available_materials = np.array(['SiO2', 'TiO2'])
    ...
    for insert_count in range(100):
        ...
        np.savetxt(f'generated_data\\10layer_0degree\\start5000nm\\count{insert_count+1}_layer{layer_number}_merit{merit}.txt', d)
        np.savetxt(f'generated_data\\10layer_0degree\\start5000nm\\count{insert_count+1}_materials.txt', materials, fmt='%s')
