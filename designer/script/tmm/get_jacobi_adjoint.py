...
def forward_and_backward_propagation(
    ...
    # make front matrix
    fill_arr(W_front_s, 0.5, 0.5 / cos_inc / n_inc, 0.5, -0.5 / cos_inc / n_inc)
    fill_arr(W_front_p, 0.5 / n_inc, 0.5 / cos_inc, 0.5 / n_inc, -0.5 / cos_inc)
    ...
    for i in range(layer_number - 1):
        ...
        calc_partial_d_M(partial_d_Ms, partial_d_Mp, cos_arr[i % 2], n_arr[i % 2], d[i], wl)
        ...
        calc_M_inv(Ms_inv, Mp_inv, cos_arr[(i + 1) % 2], n_arr[(i + 1) % 2], d[i + 1], wl)
        mul_left(Ms_inv, W_back_s) # M_0^-1 to left
        mul_left(Mp_inv, W_back_p) # M_0^-1 to left
